...
class ResumeSpider(scrapy.Spider):
	...
	def getUrl(self):
		...
		exist = session.execute("SELECT DISTINCT(state) FROM resumes").fetchall()
                ...
                # not_complete = 'select t1.cities, t2.cities, t1.state from (select count(distinct(city)) cities, state from cities group by state order by state) as t1 left join (select count(*) cities, state from resumes group by state) as t2 on t1.state=t2.state where t2.cities is not NULL and t1.cities!=t2.cities'
                states = ['AR', 'MA']
                stateStr = ''
                ...
                # result = session.execute("select t1.name, t1.state from (select distinct(city), state from cities where %s) as t1 left join resumes as t2 on (t1.name=t2.city and t1.state=t2.state) where t2.city is NULL" %(stateStr)).fetchall()
                result = session.execute("SELECT DISTINCT(city), state FROM cities").fetchall()
